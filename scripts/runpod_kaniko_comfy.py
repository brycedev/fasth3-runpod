#!/usr/bin/env python3
"""Start a RunPod CPU kaniko builder and push ghcr.io/brycedev/fasth3-runpod:comfy-larry.

Same path as FastH3: GHA GITHUB_TOKEN has packages:write. Not a GPU.
"""
...
DEST = os.environ.get("DEST", "ghcr.io/brycedev/fasth3-runpod:comfy-larry")
...
def main() -> int:
    ...
    start = (
        ...
        "--dockerfile=runpod/Dockerfile.comfy "
        f"--destination={DEST} "
    ...
    try:
        while time.time() < deadline:
            ...
            if chunk and chunk != last:
                ...
                last = chunk
            low = chunk.lower()
            if "pushed" in low and "comfy-larry" in low:
            ...
            if desired in ("EXITED", "TERMINATED") or str(status) in ("EXITED", "TERMINATED", "FAILED"):
                if "pushed" in last.lower() and "comfy-larry" in last.lower():
                    print("BUILD_OK", flush=True)
                    return 0
